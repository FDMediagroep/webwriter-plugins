...
class HistoryMainComponent extends Component {
  ...
  didMount() {
    ...
    let buttonText = this.getLabel('No thanks, create a new article');
    let description = this.getLabel('We\'ve found some unsaved articles. Click on the version you would like to restore');
    ...
    if (historyForArticle && historyForArticle.versions && historyForArticle.versions.length > 0 && api.newsItem.hasTemporaryId()) {
      ...
      buttonText = this.getLabel("Continue with last saved version");
      description = this.getLabel('We found some unsaved changes for this article.');
