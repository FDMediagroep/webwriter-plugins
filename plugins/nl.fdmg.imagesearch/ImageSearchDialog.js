...
class ImageSearchDialog extends Component {
  ...
  triggerFileUpload(ev) {
    ...
    console.log(ev.target.files, '<targetfile')
    this.send('close');
  }
  ...
  insertImageById(imageId) {
    ...
    if (command === "" || !command) {
      ...
      this._retrieveDownloadUrl(imageId)
        .then((url) => {
          console.log(url, imageId, 'url and image id || no command')
      ...
      console.log('yes command' + command)
      this._retrieveDownloadUrl(imageId)
        .then((url) => {
          console.log(url, imageId, 'url and image id || yes command')
