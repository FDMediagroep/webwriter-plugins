...
TagEditComponent.Prototype = function () {
    ...
    this.save = function () {
        ...
        if (uuid === null) {
            ...
            ajax.done(function (uuid) {
                ...
                this.context.api.addTag(this.name, {uuid: uuid, name: [fullName], type: ['person'], imType: ['x-im/person']});

                this.closeAndReload();
            }.bind(this));

        } else {
            var saveAjaxRequest = this.saveConcept(uuid, xmlTag.documentElement.outerHTML);

                saveAjaxRequest.done(function () {
    ...
    this.render = function () {
        ...
        var firstName = this.renderElement("firstNameInput", this.context.i18n.t('First name'), this.getItemMetaExtProperty('imext:firstName')['@value'], false, 'input'),
            lastName = this.renderElement("lastNameInput", this.context.i18n.t('Last name'), this.getItemMetaExtProperty('imext:lastName')['@value'], false, 'input'),
