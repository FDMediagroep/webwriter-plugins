...
class ImageDisplay extends Component {
  ...
  render($$) {
    ...
    let imgSrc = this.props.node.getUrl()
    ...
    if (imgSrc) {
      ...
      imgContainer.append($$(FontAwesomeIcon, {icon: 'fa-picture-o'}).attr('style', 'font-size:25rem;color:#efefef'))
    }
    ...
    if (imageFile.uuid && api.getConfigValue(this.props.parentId, 'imageinfo')) {
      actionsEl.append($$(Button, {icon: 'image'}).on('click', this._openMetaData))
    }

    if (imageFile.uuid && api.getConfigValue(this.props.parentId, 'softcrop')) {
      actionsEl.append($$(Button, {icon: 'crop'}).on('click', this._openCropper))
    }
